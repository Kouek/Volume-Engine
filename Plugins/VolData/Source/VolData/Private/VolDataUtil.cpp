...
TVariant<TArray<uint8>, FString> FRAWVolumeData::LoadFromFile(const LoadFromFileParameters& Params)
{
	...
	auto Transform = [&]<FVolDataVoxelType T>(T*) -> TOptional<FString> {
		...
		auto [AxisOrderMap, ReorderedVoxelPerVolume] = ReorderOpt.GetValue();

		T*			  TypedBuf = reinterpret_cast<T*>(Buf.GetData());
		decltype(Buf) OldBuf = Buf;
		T*			  TypedOldBuf = reinterpret_cast<T*>(OldBuf.GetData());
		size_t		  OldBufOffs = 0;
		size_t		  TrVoxYxX = static_cast<size_t>(ReorderedVoxelPerVolume.Y) * ReorderedVoxelPerVolume.X;
		FIntVector3	  Coord;
		for (Coord.Z = 0; Coord.Z < Params.VoxelPerVolume.Z; ++Coord.Z)
			for (Coord.Y = 0; Coord.Y < Params.VoxelPerVolume.Y; ++Coord.Y)
				...
				{
					FIntVector3 trCoord(Params.AxisOrder.X > 0 ? Coord[AxisOrderMap[0]]
															   : ReorderedVoxelPerVolume.X - 1 - Coord[AxisOrderMap[0]],
						Params.AxisOrder.Y > 0 ? Coord[AxisOrderMap[1]]
											   : ReorderedVoxelPerVolume.Y - 1 - Coord[AxisOrderMap[1]],
						Params.AxisOrder.Z > 0 ? Coord[AxisOrderMap[2]]
											   : ReorderedVoxelPerVolume.Z - 1 - Coord[AxisOrderMap[2]]);
					TypedBuf[trCoord.Z * TrVoxYxX + trCoord.Y * ReorderedVoxelPerVolume.X + trCoord.X] =
						TypedOldBuf[OldBufOffs];
					++OldBufOffs;
				}
...
TVariant<UVolumeTexture*, FString> FRAWVolumeData::CreateTexture(const CreateTextureParameters& Params)
{
...
DEFINE_LOG_CATEGORY(LogVolData)
